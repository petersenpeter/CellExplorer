...
%   License: BSD 3 clause
    ...
    properties(Constant)
        ...
        DEFAULT_A = 1.576943570285991;
        DEFAULT_B = 0.895060673920128;
    end
    ...
    methods
        ...
        function validate_parameters(U)
            ...
            if U.n_components ~= 2 && ~(strcmpi(U.method, 'MatLab') || strcmpi(U.method, 'MatLab vectorized'))
                error('The Java and C methods currently only support reducing to 2 dimensions');
            end
        ...
        function U = fit(U, X, y)
            ...
            strC=CommaFormat(C);
            if U.verbose
                log=sprintf(['Constructing fuzzy simplicial set for '...
                    '%s X %s values'], strR, strC);
                if isequal('function_handle', class(U.progress_callback))
                    feval(U.progress_callback, log);
                else
                    disp(log);
                end
            end
            if X_rows < 4096
                ...
                U.graph = fuzzy_simplicial_set(dmat, U.n_neighbors, random_state, 'precomputed',...
                ...
                U.graph = fuzzy_simplicial_set(X, U.n_neighbors, random_state, U.metric, 'metric_kwds', U.metric_kwds,...
            ...
            if ~isempty(y)
                ...
                if strcmpi(U.target_metric, 'categorical')
                    ...
                    if size(y, 1) < 4096
                        ...
                        target_graph = fuzzy_simplicial_set(ydmat, U.target_n_neighbors, random_state,...
                            'precomputed', 'metric_kwds', U.target_metric_kwds, 'set_op_mix_ratio', 1,...
                            'local_connectivity', 1);
                    else
                        target_graph = fuzzy_simplicial_set(y, U.target_n_neighbors, random_state,...
            ...
            if U.verbose
                log=sprintf('Constructing embedding for %s X %s values', ...
                    strR, strC);
                if isequal('function_handle', class(U.progress_callback))
                    feval(U.progress_callback, log);
                else
            ...
            [U.embedding, U.method] = simplicial_set_embedding(U.raw_data, U.graph, U.n_components,...
                U.initial_alpha, U.a, U.b, U.repulsion_strength, 'negative_sample_rate',...
                U.negative_sample_rate, 'n_epochs', U.n_epochs, 'init', U.init, 'metric', U.metric,...
                'metric_kwds', U.metric_kwds, 'verbose', U.verbose, 'method', U.method,...
                'progress_callback', U.progress_callback, 'epoch_reports', U.epoch_reports,...
                'probability_bin_limit', U.probability_bin_limit, 'random_state', U.random_state);
        ...
        function X_new = transform(U, X)
            ...
            if U.verbose
                if isequal('function_handle', class(U.progress_callback))
                    feval(U.progress_callback, 'Initializing template');
                else
            ...
            [sigmas, rhos] = smooth_knn_dist(dists, U.n_neighbors, adjusted_local_connectivity);
        ...
        function supervisors=setSupervisors(this, labels, labelMap, ax)
            supervisors=Supervisors(labels, labelMap, this.embedding, ax);
