...
The spike raster plots created in the Cell Explorer is not saved across sessions. To do this you have to follow the next section about predefining spike plots.

### Predefine spike plots
You can predefine custom plots that are loaded automatically in the Cell Explorer every time. Custom spike plots are located in the folder `+customSpikesPlots/`. The parameters that must be defined follows the logic described in the previous section. There is a template file (`spikes_template.m`) available in the folder to get you started:
```m
