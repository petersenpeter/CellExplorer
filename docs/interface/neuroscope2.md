...
## Opening a new session with NeuroScope2
...
```
### Open a new session form the File menu in NeuroScope2
A new dataset can also be loaded from the File menu in NeuroScope2. Select `Load session from file`, to open a file dialog and select any file from the basepath of the session you want to open, that contains the basename, e.g. `basename.session.mat`.
