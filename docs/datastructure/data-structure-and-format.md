...
CellExplorer uses one main path for each session called `basepath`. The basepath should contains the raw ephys data as well. The data in the basepath should follow this naming convention: `basename.*`, e.g. `basename.dat`. The clustered data can be in a (relative) sub directory, defined in `session.spikeSorting.relativePath` or in the basepath, all files generated by CellExplorer will be saved to the basepath. Each session is identified by a `basename`. It is recommended to follow the naming convention: `/path_to_data/basename/`, meaning that the folder containing the dataset is named `basename`, but it is not required.

### Raw data files
CellExplorer supports raw binary data files (.dat files). This raw format is also used/supported by IntanTech, OpenEphys, KiloSort, Phy, NeuroSuite, Spyking Circus, NeuroSuite, Klustakwik, and many other tools.
...
Related metadata is specified in the session struct:

* precision (numeric data type): `session.extracellular.precision`. The default precision is `int16`, but  numeric data types that Matlab supports are included: `double`, `single`, `uint16`, `int32`, `int64`
* number of channels: `session.extracellular.nChannels`
* the sampling rate: `session.extracellular.sr`
* least significant bit: `session.extracellular.leastSignificantBit`

CellExplorer also uses a `basename.lfp` file - A low-pass filtered and down-sampled raw data file for lfp analysis (for efficient data analysis and data storage; typically down-sampled to 1250Hz). The lfp file is automatically generated in the pipeline from the raw data file (using the script `ce_LFPfromDat`). The sampling rate is specified in the session struct (`session.extracellular.srLfp`). The LFP file must have the same channel count and amplification as the dat file.


## Data structures
Each type of data is saved in its own MATLAB structure, please see the list of data containers in the next section. CellExplorer is fully compatible with the [Buzcode toolbox repository](https://github.com/buzsakilab/buzcode).
