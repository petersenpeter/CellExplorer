function out = plot_behavior_xyz(varargin)
...
if isfield(data,'behavior') 
    ...
    for i = 1:numel(behaviors)
        if isfield(data.behavior.(behaviors{i}),'position') && isfield(data.behavior.(behaviors{i}).position,'x') && isfield(data.behavior.(behaviors{i}).position,'y') && isfield(data.behavior.(behaviors{i}).position,'z')
            figure,
            line(data.behavior.(behaviors{i}).position.x,data.behavior.(behaviors{i}).position.y,data.behavior.(behaviors{i}).position.z,'Marker','.','LineStyle','none','HitTest','off');
            axis tight, xlabel('x-position'), ylabel('y-position'), zlabel('z-position'), title(['Behavior: ' behaviors{i}])
        end
    end
else
