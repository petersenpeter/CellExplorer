...
if isfield(data,'cell_metrics')
    ...
    if tf
        if ~isfield(data.cell_metrics.general,'ccg')
            spindices = generateSpinDices(data.cell_metrics.spikes.times);
        ...
        for j = 1:length(plot_cells)
            for jj = 1:length(plot_cells)
                ...
                if jj == j
                    bar_from_patch_centered_bins(data.cell_metrics.general.ccg_time*1000,data.cell_metrics.general.ccg(:,plot_cells(j),plot_cells(jj)),'k')
                    title(['Cell ', num2str(plot_cells(j))]),
                    xlabel(data.cell_metrics.putativeCellType{plot_cells(j)})
                else
                    bar_from_patch_centered_bins(data.cell_metrics.general.ccg_time*1000,data.cell_metrics.general.ccg(:,plot_cells(j),plot_cells(jj)),[0.5,0.5,0.5])
                end
                ...
                try
                    if any(data.cell_metrics.putativeConnections.excitatory(:,1)==plot_cells(j) & data.cell_metrics.putativeConnections.excitatory(:,2) ==plot_cells(jj))
                    ...
                    if any(data.cell_metrics.putativeConnections.inhibitory(:,1)==plot_cells(j) & data.cell_metrics.putativeConnections.inhibitory(:,2) ==plot_cells(jj))
