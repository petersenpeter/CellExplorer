    ...
    methods(Static)
        ...
        function txtSecs=MinutesSeconds(secs)
            if secs<60
                ...
                if secs<1
                    txtSecs=[String.Pluralize2('minute', mins, 'mins')];
                else
                    mins=String.Pluralize2('minute', mins, 'mins');
        ...
        function num=encodeK(num, min)
            ...
            if num<min
                num=String.encodeNumber(num, 0);
            else
                num=round(num/1000);
                num=[String.encodeInteger(num) 'k'];
            end
            if negative
                num=['-' num];
            end
        ...
        function num=encodeBytes(num, min)
            if isempty(num) || isnan(num)
                num='n/a';
                return;
            end
            ...
            if num<1000
                num=[String.encodeNumber(num, 0) ' bytes'];
            elseif num<1000000
                num=round(num/1000);
                num=[String.encodeRounded(num, 1, true) ' KB'];
            else
                num=round(num/1000000);
                num=[String.encodeRounded(num, 1, true) ' MB'];
            end
            if negative
                num=['-' num];
            end
        ...
        function c = linewrap(s, maxchars)
            ...
            % Steven L. Eddins
            % $Revision: 1.1 $  $Date: 2019/07/08 15:51:56 $
        ...
        function [out,idxs]=sortFacs(ms, in)
            if isempty(ms)
                ms=edu.stanford.facs.swing.MarkerSorter;
            end
            a=ms.sort(in);
            N=length(a);
            out=cell(1, N);
            ...
            if nargout>1
                idxs=zeros(1,N);
                for i=1:N
                    idxs(i)=StringArray.IndexOf(in, out{i});
                end
            end
        end
