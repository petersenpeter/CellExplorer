    ...
    methods
        ...
        function H=contour(this, ax, probabilityContourPercent, ...
            ...
            if ~isempty(this.probabilityDensity)
                this.probabilityDensity.drawContours(ax, ...
        ...
        function density2D(this, data, ax, colorRangeStart, colorRangeEnd)
            ...
            if isPseudoColor
                colors=CytoGate.Get.pseudoColor;
                N=size(colors,1);
                if isempty(this.pseudoZ)
                    try
                        this.pseudoZ=this.getProbabilityDensities(N);
                    catch ex
                    end
                end
                ...
                if nargin<5 
                    ...
                    for i=2:N-1
                        for j=1:3
                            colors(i,j)=colors(1,j)+(i/N*gap(1,j));
                        end
                    end
                end
                N=length(colors);
                probabilityRange=this.getProbabilityDensities(N);
            end
            if size(data,1)<10
                color=colors(1, :);
        ...
        function [numClusts, eventClusterIds]=setLabels(this, labels, ...
                newSubsets, clusterNames, clusterColors)
