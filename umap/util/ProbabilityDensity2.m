...
classdef ProbabilityDensity2 < handle
    ...
    methods(Static)
        ...
        function fncMotion=DrawLabeled(ax, data, labels, labelMap, doContours, ...
                reset, priorFcn, doubleClicker, xMargin, yMargin)
            fncMotion=[];
            if nargin<10
                yMargin=-0.007;
                if nargin<9;
            ...
            try
                ...
                if isempty(labelMap)
                    if exist('labelMap.properties', 'file')
                        ...
                        labelMap.load(java.io.FileInputStream('labelMap.properties'));
                    end
                end
                if isempty(labelMap)
                    for i=1:N
                    ...
                    for i=1:N
                        ...
                        l=labels==u(i);
                        keyColor=[key '.color'];
                        colorString=char(labelMap.get(keyColor));
                        if ~isempty(colorString)
                            ...
                            labelMap.put(keyColor, colorString);
                        end
                        labelHs(i)=plot(ax, ...
                            data(l,1), data(l,2), '.', 'MarkerSize', 2,...
                            'MarkerEdgeColor', clr, 'LineStyle', 'none');
                        name=char(labelMap.get(key));
                        if ~isempty(name)
                            names{end+1}=name;
                            labelIdxs(end+1)=i;
                        else
                    ...
                    if ~isempty(names)
                        [~, fncMotion]=Gui.Legend(labelHs, names, ...
                            labelIdxs, xMargin, yMargin, true,...
                            [], priorFcn, doubleClicker);
