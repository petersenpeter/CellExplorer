...
classdef Gui
    ...
    methods (Static)
        ...
        function [ok, n]=SetNumberField(jtf, props, prop, low, high, ...
            ...
            if nargin>3
                if n<low
                    msg(Html.WrapHr([noun ' must be &gt;= ' num2str(low) suffix]));
                    return;
                end
                if n>high
                    msg(Html.WrapHr([noun ' must be &lt;= ' num2str(high) suffix]));
                    return;
                end
            end
            if ~isempty(props)
        ...
        function [jtf, jl, pnl]=AddNumberField(label, cols, dfltValue, ...
            ...
            if isempty(fncFmt)
                if ischar(v)
                    strVal=String.encodeRounded(str2double(v), 4, true);
                else
            ...
            function check(h, e)
                ...
                n=str2double(s);
                fprintf('CHECKING n=%s, low=%s, high=%s\n', String.encodeRounded(n,3),...
                    String.encodeRounded(low, 3), String.encodeRounded(high,3));
                if isnan(n)
                    whine=['<html>"' s '" is <font color="red"><b>not</b>'...
                        ' a number!?</font></html>'];
                elseif ~isempty(specialNum) && specialNum==n
                    whine=[];
                elseif ~isempty(low) && n<low
                        whine=[' must be >= ' num2str(low)];
                elseif ~isempty(high) && n>high
                        whine=[ ' must be <= ' num2str(high)];
                else
                    whine=[];
                end
                if isempty(whine)
                    ...
                    app.showToolTip(jtf, [whine specialMeaning]);
                end
            end
        end
        ...
        function UnderConstruction(feature, reconstruction, where, howManyTimes)
            ...
            if CytoGate.Get.Tally(feature)>howManyTimes
                return;
            end
            if nargin<3
                where='south west++';
        ...
        function [answer, cancelled, strAnswer]=Ask(msg, allLabels, ...
            ...
            if isempty(property)
                ...
                if isstruct(msg)
                    ...
                    msg.properties=CytoGate.Get;
                else
                    msg=struct('msg', msg, ...
                    'properties', CytoGate.Get, 'property', property);
                end
                [answer, cancelled]=mnuMultiDlg(msg, ttl, allLabels, ...
                    dflt, true, true, southWestButtons, 'south west buttons');
            end
        ...
        function cdata=GetImg1(file, oldColor, newColor, folder)
            if nargin<4 || isempty(folder)
                folder=CytoGate.Get.contentFolder;
            end
            if CytoGate.Get.toolBarFactor>0
                file=edu.stanford.facs.swing.Basics.GetResizedImg(...
                    java.io.File(fullfile(folder, file)), ...
                    CytoGate.Get.toolBarFactor, ...
                    java.io.File(CytoGate.Get.appFolder));
        ...
        function [H, J, H2, btnWidth]=NewBtn(txt, cb, tip, iconPath, ...
            ...
            if nargout>3
                ...
                [d, resized]=resizeJavaPrefs(CytoGate.Get, J);
                btnWidth=d.width;
                h=d.height;
                if rightAlign
                    if resized
        ...
        function [H, J]=NewBtn2(txt, cb, tip, iconPath, ...
            ...
            d=resizeJavaPrefs(CytoGate.Get, J);
        ...
        function icon=IconAnimated(file)
            ...
            if isempty(path)
                file=fullfile(CytoGate.Get.contentFolder, file);
            end
        ...
        function icon=Icon(file)
            ...
            if CytoGate.Get.toolBarFactor>0
                if length(file)<7 || ~strcmp('facs.gif', file(end-7:end))
                    file=edu.stanford.facs.swing.Basics.GetResizedImg(...
                        java.io.File(file), CytoGate.Get.toolBarFactor, ...
                        java.io.File(CytoGate.Get.appFolder));
                end
            end
            icon=javax.swing.ImageIcon(file);
        ...
        function icon=IconApp(file)
            path=fileparts(file);
            if isempty(path)
                file=fullfile(CytoGate.Get.appFolder, file);
            end
            if CytoGate.Get.toolBarFactor>0
                if length(file)<7 || ~strcmp('facs.gif', file(end-7:end))
                    file=edu.stanford.facs.swing.Basics.GetResizedImg(...
        ...
        function CascadeFigs(figs, lastOnly, topLeft, margin)
            ...
            if N>1
                if topLeft
                    [scrNum, scrPerc, scrPos]=Gui.FindScreen(gcf);
                    if isjava(figs{1})
                        ...
                        topX=scrPos(1)+margin;
                        topY=scrPos(2)+scrPos(4)-margin;
        ...
        function SetFigVisible(fig, alwaysOnTop)
            if nargin<2
                alwaysOnTop=false;
            end
...
%                designed position
        ...
        function [jl, jsc]=Label(txt, maxW, maxH)
            if nargin==0
                txt='';
            end
        ...
        function [pb, H, J]=ImageButton(iconFile, tip, callBack, fig, ...
            ...
            if isempty(p)
                iconFile=fullfile(CytoGate.Get.contentFolder, iconFile);
            end
        ...
        function [jFrame, p]=ParentFrame
            try
                if nargout==1 && ~isempty(CytoGate.Get.currentJavaWindow)
                    jFrame=CytoGate.Get.currentJavaWindow;
                    return;
                end
            catch
        ...
        function newFile=ScaleImage(file, scale, cdata, newFileName)
            ...
            if scale~=1 && scale > .1
                if nargin<4
                    ...
                    [newFile, preExists]=CytoGate.Get.getCached(...
                        fullfile(p, [n '_' s e]));
                    create=~preExists;
                else
        ...
        function [X, Y, ml]=GetCurrentPointFromJavaObj(ax, fig, ...
             ...
             Y=y-yJ;
             if CytoGate.Get.toolBarFactor>0
                 X=CytoGate.Get.toolBarFactor*X;
                 Y=CytoGate.Get.toolBarFactor*Y;
             end
...
%                disp('java already');
        ...
        function [HL, fncMotion]=Legend(plotHs, names, idxsWithName,...
                 xMargin, yMargin, doMotion, freq, priorFcn, doubleClicker)
            N=length(plotHs);
            if iscell(plotHs)
                plotHs2=zeros(1,N);
                for i=1:N
            ...
            if isempty(names) 
                names=cell(1, N);
                for i=1:N
                    cnts(i)=length(get(plotHs(i), 'XData'));
                    names{i}=get(plotHs(i), 'DisplayName');
                end
            else
                for i=1:N
                    cnts(i)=length(get(plotHs(i), 'XData'));
            ...
            for i=1:nNames
                ...
                clr=get(plotHs(idx), 'MarkerEdgeColor');
            ...
            xlim(ax,X);
            ylim(ax,Y);
            [~,I]=sort(cnts2, 'descend');
            HL=legend(ax, legendHs(I), names(I), ...
            ...
            HL.ItemHitFcn=@(h,e)Gui.LegendClick(e, legendHs, ...
                idxsWithName, plotHs, cnts2);
            ...
            function legMotion(hObj, event)
                ...
                e=get(HL,'Position');
                [normX, normY]=Gui.DataToAxNorm(ax, x, y);
                [normFigX, normFigY]=Gui.DataToFigNorm(ax, normX, normY);
                %[normX normY normFigX normFigY e]
                if normFigY<=e(2)+e(4) && normFigY>=e(2)
                    if normFigX>=e(1) && normFigX<= e(1)+e(3)
                ...
                if isempty(mdns)
                    ...
                    for ii=1:N
                        ...
                        d=[get(plotH, 'XData'); get(plotH, 'YData')]';
                        mdns(ii,:)=median(d);
                        stds(ii,:)=std(d);
                    end
                end
                [D,II]=pdist2(mdns, [x y], 'euclidean', 'Smallest', 1);
                limit=pdist2(mdns(II,:), mdns(II,:)+(stds(II,:)*3));
                if D<=limit
                    over=II;
                else
                    over=0;
                ...
                if curOver ~= over
                    ...
                    if over>0
                        ...
                        if nameIdx>0
                            if isempty(overH)
                                overH=text(normX+.03, normY+.03, names{nameIdx}, ...
                                ...
                                set(overH, 'visible', 'on', 'Position', ...
                                    [normX normY 0], 'String', names{nameIdx});
                            end
                        else
                            disp('background');
                        end
        ...
        function LegendClick(event, legendHs, idxsWithName, plotHs, nums)
            ...
            idx_=find(legendHs==event.Peer, 1);
            ...
            posX=median(get(plotH, 'XData'));
            posY=median(get(plotH, 'YData'));
            ax=get(plotH, 'Parent');
            str=get(event.Peer, 'DisplayName');
            fsz=11;
            Gui.ShowLegendTip(ax, event.Peer, posX, posY, str, fsz, 0);
        end
        
        function ShowLegendTip(ax, hLegend, posX, posY, str, fsz, secs)
        ...
        function [normX, normY]=DataToAxNorm(ax, posX, posY)
            X=xlim(ax);
            Y=ylim(ax);
