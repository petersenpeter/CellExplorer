function [cell_metrics,UI,ClickedCells] = dialog_metrics_groupData(cell_metrics,UI)
    ...
    function CreateGroupAction
        if size(UI.groupData1.sessionList.Data,2)>1
    ...
    function updateUI2
        for i = 1:numel(UI.settings.tags)
            if isfield(UI,'togglebutton')
                if  isfield(UI.groupData1,'tags') && isfield(UI.groupData1.tags,'minus_filter') && isfield(UI.groupData1.tags.minus_filter,UI.settings.tags{i})
                    UI.togglebutton.dispTags(i).Value = UI.groupData1.tags.minus_filter.(UI.settings.tags{i});
                ...
                if isfield(UI.groupData1,'tags') && isfield(UI.groupData1.tags,'plus_filter') && isfield(UI.groupData1.tags.plus_filter,UI.settings.tags{i})
                    UI.togglebutton.dispTags2(i).Value = UI.groupData1.tags.plus_filter.(UI.settings.tags{i});
    ...
    function updateGroupDataCount
        groupDataCount = [];
        if isfield(cell_metrics,UI.groupData1.groupsList{1})
            groupDataCount(1) =  numel(fieldnames(cell_metrics.(UI.groupData1.groupsList{1})));
        else
            groupDataCount(1) =  0;
        end
        if isfield(cell_metrics,UI.groupData1.groupsList{2})
            groupDataCount(2) =  numel(fieldnames(cell_metrics.(UI.groupData1.groupsList{2})));
        else
            groupDataCount(2) =  0;
        end
        if isfield(cell_metrics,UI.groupData1.groupsList{3})
            groupDataCount(3) =  numel(fieldnames(cell_metrics.(UI.groupData1.groupsList{3})));
        else
            groupDataCount(3) =  0;
        end
    ...
    function filterGroupData
        if size(UI.groupData.dataTable,1)>1
