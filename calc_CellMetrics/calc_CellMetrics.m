...
cell_metrics_type_numeric = {'spikeGroup','spikeCount','firingRate','cv2','refractoryPeriodViolation','burstIndex_Mizuseki2012','thetaModulationIndex','acg_tau_rise',...
    ...
    'thetaPhasePeak','thetaPhaseTrough','thetaEntrainment','thetaModulationIndex','spatialCoverageIndex','spatialGiniCoeff','spatialCoherence','spatialPeakRate',...
    'placeFieldsCount','placeCell','firingRateGiniCoeff','firingRateStd','firingRateInstability'};
...
if ~any(ismember(cell_metrics_fieldnames(fields_struct),cell_metrics_type_struct))
    error(['cell_metrics field not formatted correctly'])
end
% Verifying numeric type
if ~any(ismember(cell_metrics_fieldnames(cell_metrics_numeric),cell_metrics_type_numeric))
    error(['cell_metrics field not formatted correctly'])
end
% Verifying cell type
if ~any(ismember(cell_metrics_fieldnames(cell_metrics_cell),cell_metrics_type_cell))
    error(['cell_metrics field not formatted correctly'])
end
