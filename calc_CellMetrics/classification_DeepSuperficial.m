...
ripple_channels = session.Extracellular.Groups{session.ChannelTags.Ripple.SpikeGroups}+1;
...
SWR_slope = [];
for jj = 1:session.Extracellular.nGroups
    fprintf(['Analysing shank ', num2str(jj),', ']);
    
    ripple_channels{jj} = session.Extracellular.Groups{jj}+1;
    signal = 0.000050354 * double(LoadBinary([basename '.lfp'],'nChannels',nChannels,'channels',ripple_channels{jj},'precision','int16','frequency',SrLFP));
    ripple_ave2 = [];
...
figure,
for jj = 1:session.Extracellular.nGroups
    subplot(2,ceil(session.Extracellular.nGroups/2),jj)
    ...
    if ceil(session.Extracellular.nGroups/2) == jj || session.Extracellular.nGroups == jj
