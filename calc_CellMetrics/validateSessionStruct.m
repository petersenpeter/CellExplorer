function validateSessionStruct(session)
    ...
    for i = 1:numel(fields_1stLevel)
        ...
        elseif strcmp(fields_1stLevel{i},{'spikeSorting'})
            ...
            for j = 1:numel(fields_required)
                if isfield(session,'spikeSorting') && ~isempty(session.spikeSorting) && isfield(session.(fields_1stLevel{i}){1},fields_required{j}) && ~isempty(session.(fields_1stLevel{i}){1}.(fields_required{j}))
                    shortcutList = [shortcutList;{fields_required{j},['OK: ',session.(fields_1stLevel{i}){1}.(fields_required{j})]}];
                else
            ...
            for j = 1:numel(fields_optional)
                if isfield(session,'spikeSorting') && ~isempty(session.spikeSorting) && isfield(session.(fields_1stLevel{i}){1},fields_optional{j}) && ~isempty(session.(fields_1stLevel{i}){1}.(fields_optional{j}))
                    shortcutList = [shortcutList;{fields_optional{j},['OK: ', session.(fields_1stLevel{i}){1}.(fields_optional{j})]}];
                else
                    shortcutList = [shortcutList;{fields_optional{j},'<html><b><font color="blue">optional</font></b></html>'}];
                end
            end
        elseif strcmp(fields_1stLevel{i},{'epochs'})
                ...
                for k = 1:numel(session.epochs)
                    ...
                    for j = 1:numel(fields_required)
                        if isfield(session.(fields_1stLevel{i}){1},fields_required{j}) && ~isempty(session.(fields_1stLevel{i}){k}.(fields_required{j}))
                            shortcutList = [shortcutList;{'Epochs','OK'}];
                        else
                            shortcutList = [shortcutList;{fields_required{j},'<html><b><font color="red">Not defined</font></b></html>'}];
                        end
                    end
                ...
                for j = 1:numel(fields_required)
                    if isfield(session.(fields_1stLevel{i}),fields_required{j}) && ~isempty(session.(fields_1stLevel{i}).(fields_required{j}))
                        if isnumeric(session.(fields_1stLevel{i}).(fields_required{j})) && session.(fields_1stLevel{i}).(fields_required{j}) == 0
                            ...
                            value = session.(fields_1stLevel{i}).(fields_required{j});
                            if ischar(value)
                                shortcutList = [shortcutList;{fields_required{j},['OK: ' value, ]}];
                            elseif isnumeric(value)
                                shortcutList = [shortcutList;{fields_required{j},['OK: ' num2str(value), ]}];
                            else
            ...
            if isfield(session_fields.(fields_1stLevel{i}),'optional')
                ...
                for j = 1:numel(fields_optional)
                    if isfield(session.(fields_1stLevel{i}),fields_optional{j}) && ~isempty(session.(fields_1stLevel{i}).(fields_optional{j}))
                        value = session.(fields_1stLevel{i}).(fields_optional{j});
                        if ischar(value)
                            shortcutList = [shortcutList;{fields_optional{j},['OK: ' value, ]}];
                        elseif isnumeric(value)
                            shortcutList = [shortcutList;{fields_optional{j},['OK: ' num2str(value), ]}];
                        else
                            shortcutList = [shortcutList;{fields_optional{j},'OK'}];
                        end
                        %                     shortcutList = [shortcutList;{fields_optional{j},'OK'}];
            ...
            if strcmp(fields_1stLevel{i},{'extracellular'})
                if isfield(session.extracellular,'electrodeGroups') && isfield(session.extracellular.electrodeGroups,'channels') && numel([session.extracellular.electrodeGroups.channels{:}]) == session.extracellular.nChannels
                    shortcutList = [shortcutList;{'electrodeGroups ',['OK, channel count: ', num2str(numel([session.extracellular.electrodeGroups.channels{:}]))]}];
    ...
    if exist(fullfile(session.general.basePath,fileNameRaw),'file')
        shortcutList = [shortcutList;{'Raw data file',['Raw data exists: ' fileNameRaw]}];
    else
        shortcutList = [shortcutList;{'Raw data file',['<html><b><font color="red">raw data does not exist: ',fileNameRaw,'</font></b></html>']}];
    end
    
    %% Plotting figure with outcome-table
    if ismac
        dimensions = [520,(size(shortcutList,1)+1)*17.5];
    else
        dimensions = [520,(size(shortcutList,1)+1)*18.5];
    end
    ...
    fig.sessionList = uitable(fig.validateSessionStruct,'Data',shortcutList,'Position',[1, 1, dimensions(1)-1, dimensions(2)-1],'ColumnWidth',{150 350},'columnname',{'Field','Outcome'},'RowName',[],'ColumnEditable',[false false],'Units','normalized');
