...
if log_axis
    ...
    [f, Xi, u] = ksdensity(log10(X), 'bandwidth', band_width);
    Xi = 10.^Xi;
else
...
h{2} = scatter(X, drops_pos,'HitTest','off','SizeData',10,'MarkerFaceColor',color,'MarkerEdgeColor','none');
% h{2}.SizeData = 10;
...
if box_on
    ...
    box_pos = [Y(1) ((-yl(2) * box_dodge_amount) - (wdth_box * 0.3)) Y(2) - Y(1) (wdth_box * 0.6)];
        ...
        h{6} = line([Y(1) Y(4)], [(-yl(2) * box_dodge_amount) (-yl(2) * box_dodge_amount)], 'col', bxcl, 'LineWidth', line_width,'HitTest','off');
%     if box_dodge
%         
%     else
%         box_pos = [Y(1) -yl(2)/2-(wdth * 0.5) Y(2)-Y(1) wdth];
%         % mean line
%         h{4} = line([Y(3) Y(3)], [-yl(2)/2-(wdth * 0.5) -yl(2) / 2 + (wdth * 0.5)], 'col', bxcl, 'LineWidth', line_width,'HitTest','off');
%         
%         % whiskers
%         h{5} = line([Y(2) Y(5)], [-yl(2)/2 -yl(2)/2], 'col', bxcl, 'LineWidth', line_width,'HitTest','off');
%         h{6} = line([Y(1) Y(4)], [-yl(2)/2 -yl(2)/2], 'col', bxcl, 'LineWidth', line_width,'HitTest','off');
%     end
    % 'box' of 'boxplot'
    h{3} = rectangle('Position', box_pos,'HitTest','off');
