function verifySessionStruct(session)
    ...
    % Verifies required and optional fields has been set in the session struct
    ...
    % Epochs
    session_fields.epochs.required = {'name','startTime','stopTime'};
    session_fields.epochs.optional = {'behavioralParadigm'};
    ...
    % Spike sorting
    session_fields.spikeSorting.required = {'format','relativePath'};
    session_fields.spikeSorting.optional = {'method'};
    ...
    % Verifies of fields has been ste
    ...
    for i = 1:numel(fields_1stLevel)
        ...
        elseif strcmp(fields_1stLevel{i},{'spikeSorting'})
            ...
            for j = 1:numel(fields_required)
                if isfield(session.(fields_1stLevel{i}){1},fields_required{j}) && ~isempty(session.(fields_1stLevel{i}){1}.(fields_required{j}))
            ...
            fields_optional = session_fields.(fields_1stLevel{i}).optional;
            for j = 1:numel(fields_optional)
                if isfield(session.(fields_1stLevel{i}){1},fields_optional{j}) && ~isempty(session.(fields_1stLevel{i}){1}.(fields_optional{j}))
                    shortcutList = [shortcutList;{fields_optional{j},'OK '}];
                else
                    shortcutList = [shortcutList;{fields_optional{j},'<html><b><font color="blue">optional</font></b></html>'}];
                end
    ...
    % verify if data that is pointed to exists
    shortcutList = [shortcutList;{['<html><b>','Verifying paths and fields','</b></html>'], ''}];
