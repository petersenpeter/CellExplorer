...
% Loading existing session.mat file if exist
if exist(fullfile(basepath,'session.mat'),'file')
    load(fullfile(basepath,'session.mat'))
else
    session = [];
end
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
...
temp = dir('Kilosort_*');
if ~isempty(temp)
    session.general.clusteringPath = fullfile(basepath,temp.name); % clusteringPath assumed from Kilosort
else
