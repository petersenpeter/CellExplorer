...
if ~isfield(session,'animal')
    ...
    session.animal.sex = ''; % Male, Female, Unknown
    session.animal.species = 'Human'; % Mouse, Rat
    session.animal.strain = '';
...
if ~isfield(session,'extracellular') || (isfield(session,'extracellular') && (~isfield(session.extracellular,'sr')) || isempty(session.extracellular.sr))
    ...
    session.extracellular.fileName = 'data_all.raw';        % (optional) file name of raw data if different from basename.dat
...
if ~isfield(session,'spikeSorting')
    ...
    kiloSortFolder = dir('Kilo_*'); 
    SpykingCircusFolder = dir('data_*'); 
    ...
    elseif ~isempty(SpykingCircusFolder)
        session.spikeSorting{1}.relativePath = SpykingCircusFolder.name;
    else
...
if ~isfield(session,'analysisTags') || (isfield(session,'analysisTags') && (~isfield(session.analysisTags,'probesLayout')) || isempty(session.analysisTags.probesLayout)) 
    ...
    session.analysisTags.chanMapFile = 'MEA_ChanMap.mat';
end
